...
src.indexWatch = ['./' + dirs.src + '/pug/**/*.pug'];
...
gulp.task(
	'htmlChange',
	gulp.series('pugDist', 'inlineMIN', (done) => {
...
gulp.task(
	'js',
	gulp.series('jsPrettier', 'jsDist', 'jsOut', (done) => {
...
gulp.task(
	'sass',
	gulp.series('sassPrettier', 'sassDist', (done) => {
...
gulp.task('compile-start', (done) => {
	smallText(
		'OPTIMIZING AND COMPILING ALL SRC ASSETS...',
		['#FA7483', '#3D4583'],
		'left'
	);
	done();
...
gulp.task(
	'compile-references',
	gulp.parallel('pug', 'js', 'sass', (done) => done())
);
gulp.task(
	'compile',
	gulp.series('compile-start', 'copy', 'compile-references', (done) => done())
);
gulp.task(
	'compile-code',
	gulp.series('compile-start', 'compile-references', (done) => done())
);

gulp.task('sizeCheck', function() {
	return gulp.src(output.sizeCheck).pipe(sizeReport());
});
...
gulp.task(
	'default',
	gulp.series('intro', 'clean', 'compile', 'inline', 'sizeCheck', 'serve')
);
gulp.task('all', gulp.series('default'));
gulp.task('code', gulp.series('compile-code', 'inline', 'sizeCheck', 'serve'));
